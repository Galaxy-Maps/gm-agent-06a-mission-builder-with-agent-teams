...
version: 1.0.0
author: Galaxy Maps
repository: https://github.com/Galaxy-Maps/gm-agent-06a-mission-builder-with-agent-teams
standalone: true
mode: agent-team
teamSize: dynamic
model: high
...
| Tool | Purpose |
|------|---------|
| `Read` | Read INTENT.md and the finalized `MAP_V{final}.md` |
| `Write` | Each teammate writes their star's mission `.md` and `.html` files |
| `Bash` | `git add missions/ && git commit` (at cleanup) |
| `TeamCreate` / `TeamDelete` | Open/close the dynamic-size Agent Team (1 teammate per star) |
| `Task` | Spawn one builder teammate per star |
| `SendMessage` | Teammates coordinate terminology, running examples, and visual style across star boundaries |
| `WebFetch` / `WebSearch` | *(Optional)* teammates research concrete examples, current API refs, or real-world applications |

The HTML output assumes the consuming app provides the Galaxy Maps stylesheet (selectors like `.mission-content`, `.callout`, `.try-it` etc. — see CSS Classes Reference below).

---

## Standalone Usage

When invoked by the orchestrator (`gm-agent-01a-orchestrator-with-agent-teams`), the orchestrator handles the team lifecycle below. When invoked directly (e.g. `Skill({ skill: "gm-agent-06a-mission-builder-with-agent-teams" })`), this skill runs the team itself:

1. `Read` `INTENT.md` and `MAP_V{final}.md` to determine star count `N`.
2. `TeamCreate({ team_name: "mission-builder-team" })`.
3. Spawn one teammate per star with `Task({ name: "star-{n}", team_name, subagent_type: "general-purpose", prompt: ... })` using the Spawn Prompt Template below.
4. Each teammate first submits a content outline to you (plan approval). Approve when the approach is sound and consistent with siblings; otherwise message back with revisions.
5. Once plans are approved, teammates generate their star's missions and self-coordinate terminology / running examples / visual style via `SendMessage`.
6. `TeamDelete()` and commit: `git add missions/ && git commit -m "feat(missions): add all mission content"`.

There are no role files — teammates differ only by their assigned star.

In standalone mode, "the lead" is you (this skill's invoking session).
